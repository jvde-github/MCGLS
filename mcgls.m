...
%     Preprint 1265, Dep. Math., University Utrecht, 
%     Utrecht, the Netherlands, January, 2003
...
global A_operator Operator_params
  ...
  if na == 0
    ...
    %%% find the right hand side vector and the dimension n of the problem
    b=varargin{2}; 
    [nb,mb]=size(b);
    if n > 0 
      if nb ~= m | mb ~=1
        msg=sprintf(' The right hand side vector b')
        msg=sprintf('%s\n is %i by %i, b must be %i by 1.',msg,nb,mb,n);
